...
laser_pub = rospy.Publisher('filtered_scan', sensor_msgs.msg.LaserScan, queue_size=10)
...
def callback(data):
	...
	for idx in range(0, pc_length): # 필터링 된 pointcloud2 점 데이터 각각의 각도 계산
		scan_angle[idx] = math.atan2(data['y'][idx], data['x'][idx])

	for index in range(0, size):
		# print(index)
		current_angle = -3.14 + index * scan.angle_increment
		tmp_idx = min_diff_pos(scan_angle, current_angle) # scan_angle 리스트에서 가장 유사한 값에 대한 index를 리턴한다.
